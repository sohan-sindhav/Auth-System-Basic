...
const Login = () => {
  const { user, Login, msg, success, loading, setMsg } = useAuth();
  ...
  return (
    <div>
      <form
        ...
        className="h-screen justify-center flex items-center bg-[#222222] flex-col"
      >
        <div className="login-container">
          <h1 className="text-white text-2xl  bg-[#101010] p-2 text-center rounded-t">
            Login
          </h1>
          <div className="form-container bg-[#141414] p-14 text-white flex flex-col gap-4 rounded  ">
            {msg?.msg && (
              <p
            ...
            <button
              ...
              disabled={loading}
            >
              <div> {loading ? "Loggin in..." : "Login"}</div>
              <div className="flex justify-center items-center">
                {" "}
                {loading && <Spinner />}
              </div>
            </button>
            <div className="flex justify-center gap-1 mt-4 -mb-7">
              Not Registered ?
              <Link className="underline text-cyan-500" to="/Register">
                Register here
              </Link>
            </div>
          </div>
